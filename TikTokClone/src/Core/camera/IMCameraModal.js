...
const defaultVideoMaxDuration = 60
...
export default function IMCameraModal(props) {
    ...
    if (cameraRef.current) {
      try {
        ...
        const videoRecordPromise = cameraRef.current.recordAsync({
          ...
          quality: '720p',
          maxDuration: maxDuration || defaultVideoMaxDuration
        });
  ...
  const onOpenPhotos = async () => {
    ...
    let mediaFile = await ImagePicker.launchImageLibraryAsync({
      ...
      videoMaxDuration: maxDuration || defaultVideoMaxDuration
    });
  ...
  const onVideoSpeedChange = (videoSpeed, videoRate) => {
    setSelectedVideoSpeed(videoSpeed)
